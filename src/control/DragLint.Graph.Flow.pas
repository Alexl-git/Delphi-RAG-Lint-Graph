...
type
  ...
  TFlowBuilder = class
    strict private
      ...
      function AddStep(const AId: string; ADepth, ACallLine: Integer; const AAncestors: TArray<string>): Integer;
    ...
    public
      ...
      constructor Create(const ASource: IFlowSource; AMaxDepth: Integer = 6; AMaxBreadth: Integer = 12);
      ...
      function Build(const ARootId: string; const AExpanded: TArray<string> = nil): TFlowTree;
  end;

implementation

constructor TFlowBuilder.Create(const ASource: IFlowSource; AMaxDepth, AMaxBreadth: Integer);
begin
  inherited Create;
  FSource    := ASource;
  FMaxDepth  := AMaxDepth;
...
function TFlowBuilder.DedupOrder( const ACallees: TArray<TFlowCallee>): TArray<TFlowCallee>;
var
...
function TFlowBuilder.AddExternal(const ACallee: TFlowCallee; ADepth: Integer): Integer;
var
  X: TFlowStep;
begin
...
function TFlowBuilder.AddStep(const AId: string; ADepth, ACallLine: Integer; const AAncestors: TArray<string>): Integer;
var
  Step   : TFlowStep          ;
  Info   : TFlowInfo          ;
...
begin
  ...
  try
    ...
    begin
      if Callees[I].SymbolId = '' then Kids.Add(AddExternal(Callees[I], ADepth + 1))
      else Kids.Add(AddStep(Callees[I].SymbolId, ADepth + 1, Callees[I].CallLine, Anc));
    end;
  ...
  if Length(Callees) > Take then Step.TruncatedChildren:= Length(Callees) - Take;

  FSteps[Result]:= Step; { write back children + truncation }
end; // function

function TFlowBuilder.Build(const ARootId: string; const AExpanded: TArray<string>): TFlowTree;
var
  S: string;
begin
  ...
  try
    for S in AExpanded do FExpanded.AddOrSetValue(S, True);
