...
type
  ...
  public
    ...
    function StepVisible(AData: TGraphData; const AVisibleIdx: TArray<Integer>;
      const AEdgeSrc, AEdgeDst: TArray<Integer>;
      AIterations: Integer = 1): Boolean;
...
begin
  ...
  FHeight := AHeight;
  Area    := AWidth * AHeight;
  if AVisibleCount > 0 then
    FK := Sqrt(Area / AVisibleCount)
  else
    FK := 50.0;
  FTemperature := AWidth / 10.0;
end;

function TGraphLayout.StepVisible(AData: TGraphData;
  const AVisibleIdx: TArray<Integer>; const AEdgeSrc, AEdgeDst: TArray<Integer>;
  AIterations: Integer): Boolean;
var
  Iter, I, J: Integer;
  A, B: PGraphNode;
